...
from bregman import jump_kernel_elbo
...
class AnyOrderMaskInsertionInterpolant(JointInterpolant):
    ...
    def training_loss(self, model, x1, t):
        interpolant_sample = self.sample_interpolant(t, x1)
        unmask_weight, insert_weight = self.elbo_weight(t, x1)

        prediction: ModelPrediction = model(interpolant_sample.xt, t)

        scale_factor = x1.shape[0] * self.max_length
    ...
    def compute_loss(self, model, batch):
        ...
        unmask_loss, len_loss, loss = self.training_loss(model, x1, t)
